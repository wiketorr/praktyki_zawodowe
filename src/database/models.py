...
class register_data(
    ...
    def validate_password(cls, password: str, username: str) -> SecretStr:
        numbers = 0
        special_char = 0
        upper_case = 0
        if len(password) < 8:
...
class app_profile_data(
    BaseModel
):  # We need a model that will represent user profile in the app.
    id: int
    username: str
    email: EmailStr
    password: SecretStr
...
user_1 = register_data(
    username="wiketorr", email="wiktor@gmail.com", password="Haslo_123"
)
